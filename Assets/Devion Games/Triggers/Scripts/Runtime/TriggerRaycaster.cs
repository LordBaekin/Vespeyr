...
{
    /// <summary>
    /// This component raycasts from camera and sends OnCameraRaycast events to DisplayName components.
    /// </summary>
    public class TriggerRaycaster : MonoBehaviour
    {
        ...
        static void Initialize()
        {
            ...
            if (camera == null)
            {
                // Fallback: any enabled camera in the scene
            ...
            if (camera == null)
            {
                Debug.LogWarning("TriggerRaycaster.Initialize: No Camera found to attach to.");
                return;
            }
        ...
        private void Update()
        {
            ...
            if (TriggerRaycaster.Raycast(ray, out hit, float.PositiveInfinity, this.m_LayerMask))
            {
                ...
                if (Input.GetMouseButtonDown(2)) button = 2;
