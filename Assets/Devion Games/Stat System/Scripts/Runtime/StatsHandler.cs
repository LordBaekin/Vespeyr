    ...
    public class StatsHandler : MonoBehaviour, IJsonSerializable
    {
       ...
       /* private void OnGUI()
        ...
        public void ApplyDamage(string name, float damage)
        {
            ...
            // ────── DEBUG LOGGING ──────
            Debug.Log($"[StatsHandler] ({HandlerName}) ApplyDamage('{name}', {damage}). "
                    + $"Before: CurrentValue={stat.CurrentValue} / Threshold={stat.Value}");
            ...
            Debug.Log($"[StatsHandler] ({HandlerName}) '{name}' after ApplyDamage: CurrentValue={currentValue}");
        }
        ...
        public void SetObjectData(Dictionary<string, object> data)
        {
            ...
            var statList = data["Stats"] as List<object>;
            Debug.Log($"[StatsHandler] ({HandlerName}) SetObjectData called with {statList.Count} entries");

            for (int i = 0; i < statList.Count; i++)
            {
                var statData = statList[i] as Dictionary<string, object>;
                if (statData == null) continue;

                Stat stat = GetStat((string)statData["Name"]);
                if (stat == null) continue;

                // restore baseValue (via Stat) and currentValue (via Attribute)
                stat.SetObjectData(statData);

                // ────── DEBUG LOGGING ──────
                if (stat is Attribute attr)
                {
                    Debug.Log($"[StatsHandler] ({HandlerName}) Restored Attribute '{stat.Name}': "
                            + $"Threshold(Value)={stat.Value}, CurrentValue={attr.CurrentValue}");
                }
                else
                {
                    Debug.Log($"[StatsHandler] ({HandlerName}) Restored Stat '{stat.Name}': Value={stat.Value}");
                }
            }
        }
