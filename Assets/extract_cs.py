...
Recursively find all .cs files under the current directory (or a given one),
...
def zip_cs_files(src_dir: str, zip_path: str) -> None:
...
def main():
    parser = argparse.ArgumentParser(
        description="Zip all .cs files from the current (or a specified) directory."
    )
    parser.add_argument(
        '-s', '--source',
        default='.',
    ...
    zip_cs_files(args.source, args.output)
